(function () {
	...
	/* istanbul ignore next */
	const uint32NoCrypto = () => ceil((rand() * BITS32) >>> 0);
	...
	*/
	...
	class WebsocketProvider extends Observable {
	   ...
	   */
	  constructor (url, roomname, doc, { awareness = new Awareness(doc), db = null } = /** @type {any} */ ({})) {
	    super();
	  ...
	  connect () {
	    ...
	    if (!this.wsconnected && this.ws === null) {
	      ...
	      this.mux(() => {
	        ...
	        publish(this.url, toUint8Array(encoderState));
	        // write queryAwareness
	        const encoderAwarenessQuery = createEncoder();
	        writeVarUint(encoderAwarenessQuery, messageQueryAwareness);
	        publish(this.url, toUint8Array(encoderAwarenessQuery));
	        // broadcast local awareness state
	        const encoderAwarenessState = createEncoder();
	        writeVarUint(encoderAwarenessState, messageAwareness);
	        writeVarUint8Array(encoderAwarenessState, encodeAwarenessUpdate(this.awareness, [this.doc.clientID]));
	        publish(this.url, toUint8Array(encoderAwarenessState));
	      });
	...
	const codemirrorCursorActivity = (y, cm, type, awareness) => {
	  ...
	  if (aw.cursor != null) {
	    currentAnchor = createAbsolutePositionFromRelativePosition(JSON.parse(aw.cursor.anchor), y);
	    currentHead = createAbsolutePositionFromRelativePosition(JSON.parse(aw.cursor.head), y);
	  }
